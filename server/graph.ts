...
async function getAccessToken(tenant: GraphTenant) {
  const config = configFor(tenant);
  ...
  if (!response.ok) throw new Error(`Graph token request failed with ${response.status}`);
...
export async function checkGraphHealth(tenant: GraphTenant): Promise<GraphHealth> {
  const config = configFor(tenant);
  if (!isGraphConfigured(tenant)) {
    return {
      tenant,
      configured: false,
      status: "not_configured",
      tenantId: config.tenantId || undefined,
      detail: "Add tenant ID, client ID, and client secret to enable live Microsoft Graph access.",
    };
  }
  try {
    ...
    return {
      ...
      detail: "Microsoft Graph client-credentials connection is healthy.",
    };
  } catch (error) {
    return {
      tenant,
      ...
      status: "error",
      tenantId: config.tenantId,
      detail: error instanceof Error ? error.message : "Microsoft Graph health check failed.",
    };
  }
}
