...
export const migrationJobs: MigrationJob[] = [
  { id: "JOB-2408", name: "Finance pilot wave", workload: "OneDrive", owner: "Workspace admin", status: "Running", progress: 76, items: "184 / 242 users", started: "Today, 08:42", throughput: "118 GB/hr", errors: 4, eta: "~1h 26m" },
...
export const discoveryUsers = [
  { initials: "AR", name: "Alicia Rodriguez", email: "alicia.rodriguez@source-a.example", source: "Source tenant A", target: "alicia.rodriguez@destination.example", workload: "OneDrive · Mailbox", status: "Mapped", risk: "Low" },
  { initials: "JM", name: "Jordan Miller", email: "jordan.miller@source-a.example", source: "Source tenant A", target: "jordan.miller@destination.example", workload: "OneDrive", status: "Mapped", risk: "Low" },
  { initials: "KN", name: "Kaitlyn Nguyen", email: "kaitlyn.nguyen@source-a.example", source: "Source tenant B", target: "—", workload: "OneDrive · Teams", status: "Needs mapping", risk: "Medium" },
  { initials: "OM", name: "Owen McCarthy", email: "owen.mccarthy@source-a.example", source: "Source tenant A", target: "owen.mccarthy@destination.example", workload: "OneDrive · SharePoint", status: "Mapped", risk: "Low" },
  { initials: "RD", name: "Renee Dubois", email: "renee.dubois@source-a.example", source: "Source tenant B", target: "renee.dubois@destination.example", workload: "OneDrive", status: "Conflict", risk: "High" },
  { initials: "TS", name: "Tomas Silva", email: "tomas.silva@source-a.example", source: "Source tenant C", target: "—", workload: "OneDrive", status: "Excluded", risk: "Low" },
];

export const errorRows = [
  { code: "OD-403", message: "Target user does not have an active OneDrive license", entity: "renee.dubois@destination.example", job: "JOB-2405", severity: "High", lastSeen: "8 min ago", action: "Assign license" },
  ...
  { code: "MAP-102", message: "Multiple target matches returned for source identity", entity: "kaitlyn.nguyen@source-a.example", job: "Discovery", severity: "Medium", lastSeen: "26 min ago", action: "Review mapping" },
  { code: "OD-404", message: "Source personal site could not be resolved", entity: "tomas.silva@source-a.example", job: "JOB-2407", severity: "Low", lastSeen: "41 min ago", action: "Retry discovery" },
];

export const auditEvents = [
  { time: "09:42:18", actor: "Workspace admin", action: "Resumed migration job", scope: "JOB-2408", result: "Success", ip: "10.24.8.14" },
